...
Adafruit_MotorShield ArmShield(0x62);
Adafruit_MotorShield ContainShield(0x61);
Adafruit_MotorShield DoorShield(0x60);

Adafruit_StepperMotor *lArmStepper = ArmShield.getStepper(200, 2);   //left motor while looking from end with servos
//Adafruit_StepperMotor *rArmStepper = ArmShield.getStepper(200, 1);   //right motor from same orientation as above
...
Serial.begin(9600);
 //pinMode(13, OUTPUT);
  ...
  lArmStepper->setSpeed(rpm);   //20 rpm
...
void loop() {
  ...
  if(Serial.available()>0){
   // digitalWrite(13, LOW);
    ...
    } else if(incomingByte=='0'){               //NO BUTTON IS PRESSED
      delay(1);
    } //else {
